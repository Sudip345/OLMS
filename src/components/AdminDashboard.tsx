...
export const AdminDashboard: React.FC = () => {
  const { lotteries, transactions, addLottery, removeLottery, addTransaction } = useLottery();
  ...
  const selectWinner = (lotteryId: string) => {
    ...
    if (lottery.prizeDistributed) {
      toast.error('Prize has already been distributed for this lottery!');
      return;
    }
    ...
    const updatedLotteries = lotteries.map(l => 
      l.id === lotteryId ? { ...l, winner: winnerEmail, prizeDistributed: true } : l
    );
    ...
    const winnerData = JSON.parse(localStorage.getItem(`userData_${winnerEmail}`) || '{}');
    localStorage.setItem(`userData_${winnerEmail}`, JSON.stringify({
      ...winnerData,
